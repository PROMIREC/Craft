...
export async function POST(_req: Request, ctx: { params: { projectId: string } }) {
  try {
    ...
    if (meta.pspec.latest_revision < 1) {
      return NextResponse.json({ ok: false, error: "No PSPEC to reject." }, { status: 400 });
    }
    const now = new Date().toISOString();
    meta.updated_at = now;
    const rev = meta.pspec.latest_revision;
    meta.pspec.approval = { state: "rejected", revision: rev, decided_at: now };
    const entry = meta.pspec.revisions.find((r) => r.revision === rev);
    if (entry) entry.approval = { state: "rejected", decided_at: now };
